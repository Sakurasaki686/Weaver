...
FIntVector AVoxelWorld::WorldLocationToChunkID(FVector WorldLocation) const
{
	...
	const int32 X = FMath::FloorToInt((WorldLocation.X + Epsilon) / ChunkWorldSize);
	const int32 Y = FMath::FloorToInt((WorldLocation.Y + Epsilon) / ChunkWorldSize);
	const int32 Z = FMath::FloorToInt((WorldLocation.Z + Epsilon) / ChunkWorldSize);
...
void AVoxelWorld::SculptAtLocation(FVector WorldLocation, float BrushRadius, float BrushStrength)
{
	...
	FIntVector ChunkID = WorldLocationToChunkID(WorldLocation);
	UVoxelChunk* TargetChunk = GetOrCreateChunkByID(ChunkID);
	if (!TargetChunk)
	{
		return;
	}

	UVoxelBrush* Brush = NewObject<UVoxelBrush>(this, UVoxelBrush::StaticClass());
	if (!Brush)
	{
		return;
	}

	Brush->Location = WorldLocation / VoxelWorldSize;
	Brush->Shape = NewObject<USphereShape>(this, USphereShape::StaticClass());
	if (USphereShape* SphereShape = Cast<USphereShape>(Brush->Shape))
	{
